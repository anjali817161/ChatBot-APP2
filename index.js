var data= {
    ...
    news: {
        ...
        url: {
            ...
            link: [
                ...
                "https://m.dailyhunt.in/news/india/english/for+you?launch=true&mode=pwa",
                "https://epaper.livehindustan.com/",
                "https://m.dailyhunt.in/news/india/english/for+you?launch=true&mode=pwa"
            ]
        }
    ...
    music:{
         ...
         options:["Hindi","Punjabi","English","Pop Up","Party Mix"],
         url:{
...
function handleOptions(options, url) {
    for (var i = 0; i < options.length; i++) {
        var opt = document.createElement("span");
        var link = url.link && url.link[i] ? url.link[i] : url.more; // Fallback to url.more
        var inp = `<a class="m-link" href="${link}" target="_blank">${options[i]}</a>`;
        opt.innerHTML = inp;
        opt.setAttribute("class", "opt");
        cbot.appendChild(opt);
    }
    ...
    if (url.more) {
        var opt = document.createElement("span");
        var inp = `<a class="m-link" href="${url.more}" target="_blank">See more</a>`;
        opt.innerHTML = inp;
